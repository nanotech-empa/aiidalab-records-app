    ...
    def load_sketch_cdxml(self, change):
        for filename, data in self.structure_file_uploader.value.items():
            ...
            if generate_molecule_image == False:
                self.structure_sketch_imagebox.value = utils.read_file("./data/images/white_screen.jpg")
...
class ExperimentObjectWidget(ipw.VBox):
    ...
    def load_precursor_cdxml(self, change):
        for filename, data in self.precursor_cdxml_uploader.value.items():
            ...
            if generate_molecule_image == False:
                self.precursor_imagebox.value = utils.read_file("./data/images/white_screen.jpg")
                print(f"Cannot generate molecule image.")
...
class RecordObjectWidget(ipw.VBox):
        ...
        for experiment in experiments_objects:
            ...
            if experiment.spin_excitation_energy_ev_textbox.value:
                ...
                if utils.is_number(spin_excitation_energy_ev):
                    ...
                    print(f"Spin excitation energy (eV) is not a number")
            ...
            if experiment.gap_ev_textbox.value:
                ...
                if utils.is_number(gap_ev):
                    ...
                    print(f"Gap (eV) is not a number")
    ...
    def load_widgets(self, record_content):
        ...
        self.summary_info_widgets.names_textbox.value = ';'.join(map(str, record_content["names"]))
        self.summary_info_widgets.structure_sketch_imagebox.value = utils.read_file(record_content["structure_sketch_file"])
        ...
        self.summary_info_widgets.classification_textbox.value = record_content["classification"]
        ...
        for idx, simulation_content in enumerate(simulations_data):
            ...
            simulation_object.uks_multi_textbox.value = str(simulation_content.get("uks_multiplicity", ""))
            simulation_object.charge_textbox.value = str(simulation_content.get("charge", ""))
            simulation_object.num_alpha_textbox.value = str(simulation_content.get("number_alpha_electrons", ""))
            simulation_object.num_beta_textbox.value = str(simulation_content.get("number_beta_electrons", ""))
            simulation_object.energy_au_textbox.value = str(simulation_content.get("energy_au", ""))
            simulation_object.energy_ev_textbox.value = str(simulation_content.get("energy_ev", ""))
            simulation_object.gap_ev_textbox.value = str(simulation_content.get("gap_ev", ""))
            ...
            for idx, band_energy in enumerate(band_energies_data):
                ...
                if idx > 0:
                    band_energies_grid_box_items = list(simulation_object.band_energies_gridbox.children)
                    band_energies_grid_box_items.extend(
                        [
                            utils.Text(
                                layout = ipw.Layout(width = "80%")
                    ...
                    simulation_object.band_energies_gridbox.children = band_energies_grid_box_items
                
                table_idx = (idx + 1) * 4
                simulation_object.band_energies_gridbox.children[table_idx].value = index
                simulation_object.band_energies_gridbox.children[table_idx+1].value = label
                simulation_object.band_energies_gridbox.children[table_idx+2].value = e_ef_ev
                simulation_object.band_energies_gridbox.children[table_idx+3].value = effective_mass_m_e
            ...
            for idx, ldos_orbital_map in enumerate(ldos_orbital_maps):
                ...
                caption_widget = utils.Text(
                    description = "Caption: ",
                    value = caption
                )
                
                widgets_vbox = utils.VBox(
                    [
                        image_widget,
                        caption_widget
                    ]
            ...
            simulation_object.ldos_orbital_maps_widgets.children = ldos_orbital_maps_widgets
        ...
        for idx, experiment_content in enumerate(experiments_data):
            ...
            experiment_object.comments_textareabox.value = experiment_content["comments_flags"]
            ...
            experiment_object.spin_excitation_energy_ev_textbox.value = str(experiment_content["spin_excitation_energy_ev"])
            ...
            for idx, band_energy in enumerate(band_energies_data):
                ...
                if idx > 0:
                    band_energies_grid_box_items = list(experiment_object.band_energies_gridbox.children)
                    band_energies_grid_box_items.extend(
                        [
                            utils.Text(
                                layout = ipw.Layout(width = "80%")
                    ...
                    experiment_object.band_energies_gridbox.children = band_energies_grid_box_items
                
                table_idx = (idx + 1) * 6
                experiment_object.band_energies_gridbox.children[table_idx].value = index
                experiment_object.band_energies_gridbox.children[table_idx+1].value = label
                experiment_object.band_energies_gridbox.children[table_idx+2].value = e_ef_ev
                experiment_object.band_energies_gridbox.children[table_idx+3].value = error_ev
                experiment_object.band_energies_gridbox.children[table_idx+4].value = effective_mass_m_e
                experiment_object.band_energies_gridbox.children[table_idx+5].value = error_m_e
            ...
            experiment_object.relevant_stm_image_widgets.children = relevant_stm_image_widgets
            ...
            experiment_object.relevant_sts_image_widgets.children = relevant_sts_image_widgets

            # Precursor images
            precursor_widgets = []
            precursor_content = experiment_content.get("precursor", {})
            if precursor_content:
                experiment_object.precursor_imagebox.value = utils.read_file(precursor_content["image_file"])
                ...
                experiment_object.precursor_caption_textbox.value = precursor_content["caption"]
