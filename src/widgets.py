...
class SummaryObjectWidget(ipw.VBox):
    def __init__(self):
        ...
        self.names_label = utils.Label(value="Names: ")
        self.names_textbox = utils.Text(
            layout=ipw.Layout(width="30%"), placeholder="Write names separated by ;"
        )

        self.names_widgets = utils.HBox([self.names_label, self.names_textbox])

        self.structure_file_uploader_label = utils.Label(value="Upload structure:")

        self.structure_file_uploader = utils.FileUpload(accept=".cdxml", multiple=False)

        self.structure_file_uploader_widgets = utils.HBox(
            [self.structure_file_uploader_label, self.structure_file_uploader]
        )

        self.structure_sketch_label = utils.Label(value="Structure sketch: ")
        ...
        self.structure_sketch_widgets = ipw.HBox(
            [self.structure_sketch_label, self.structure_sketch_imagebox]
        )

        self.structure_file_label = utils.Label(value="Structure file:")

        self.structure_file_hyperlink = utils.HTML(value="")

        self.structure_cdxml_file = ""

        self.structure_file_widgets = utils.HBox(
            [self.structure_file_label, self.structure_file_hyperlink]
        )

        self.classification_label = utils.Label(value="Classification: ")

        self.classification_textbox = utils.Text(layout=ipw.Layout(width="30%"))

        self.classification_widgets = utils.HBox(
            [self.classification_label, self.classification_textbox]
        )

        self.structure_file_uploader.observe(self.load_sketch_cdxml)
    ...
    def load_sketch_cdxml(self, change):
        for filename, data in self.structure_file_uploader.value.items():
            ...
            cdxml_filepath = f"./data/structures/{filename}"
            ...
            if len(molecules) == 1:
                ...
                if chem_mol is not None:
                    ...
                    image_filepath = f"./data/images/{mol_name}.png"
                    image.save(image_filepath)
                    self.structure_sketch_imagebox.value = image_bytes
                    self.structure_sketch_image_file = image_filepath

            if generate_molecule_image is False:
                self.structure_sketch_imagebox.value = utils.read_file(
                    "./data/images/white_screen.jpg"
                )
...
class SimulationObjectWidget(ipw.VBox):
    def __init__(self, accordion_parent_widget, accordion_parent_item_index):
        ...
        self.substrate_label = utils.Label(value="Substrate: ")

        self.substrate_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.substrate_widgets = utils.HBox(
            [self.substrate_label, self.substrate_textbox]
        )

        self.uks_multi_label = utils.Label(value="UKS Multi.: ")

        self.uks_multi_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.uks_multi_widgets = utils.HBox(
            [self.uks_multi_label, self.uks_multi_textbox]
        )

        self.charge_label = utils.Label(value="Charge: ")

        self.charge_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.charge_widgets = utils.HBox([self.charge_label, self.charge_textbox])

        self.num_alpha_label = utils.Label(value="Number of alpha: ")

        self.num_alpha_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.num_alpha_widgets = utils.HBox(
            [self.num_alpha_label, self.num_alpha_textbox]
        )

        self.num_beta_label = utils.Label(value="Number of beta: ")

        self.num_beta_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.num_beta_widgets = utils.HBox([self.num_beta_label, self.num_beta_textbox])

        self.energy_au_label = utils.Label(value="Energy [au]: ")

        self.energy_au_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.energy_au_widgets = utils.HBox(
            [self.energy_au_label, self.energy_au_textbox]
        )

        self.energy_ev_label = utils.Label(value="Energy [eV]: ")

        self.energy_ev_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.energy_ev_widgets = utils.HBox(
            [self.energy_ev_label, self.energy_ev_textbox]
        )

        self.gap_ev_label = utils.Label(value="Gap [eV]: ")

        self.gap_ev_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.gap_ev_widgets = utils.HBox([self.gap_ev_label, self.gap_ev_textbox])

        self.band_energies_label = utils.Label(value="Band energies (wrt E_F)")

        self.band_energies_items = [
            utils.Label(value="Index"),
            utils.Label(value="Label"),
            utils.Label(value="E - E_F (eV)"),
            utils.Label(value="Effective mass (m_e)"),
            utils.Text(layout=ipw.Layout(width="80%")),
            utils.Text(layout=ipw.Layout(width="80%")),
            utils.Text(layout=ipw.Layout(width="80%")),
            utils.Text(layout=ipw.Layout(width="80%")),
        ]
        ...
        self.add_row_band_energies_button = utils.Button(
            button_style="success", icon="plus", layout=ipw.Layout(width="5%")
        )

        self.remove_row_band_energies_button = utils.Button(
            button_style="danger", icon="minus", layout=ipw.Layout(width="5%")
        )

        self.change_rows_band_energies_buttons = utils.HBox(
            [self.add_row_band_energies_button, self.remove_row_band_energies_button]
        )

        self.ldos_orbital_maps_label = utils.Label(value="LDOS/Orbital Maps")

        self.ldos_orbital_maps_imagebox = utils.Image(
            layout=ipw.Layout(border="solid 1px #cccccc", width="250px", height="250px")
        )

        self.ldos_orbital_maps_image_uploader = utils.FileUpload(multiple=True)
        ...
        self.add_row_band_energies_button.on_click(self.add_band_energies_row)

        self.remove_row_band_energies_button.on_click(self.remove_band_energies_row)

        self.remove_simulation_button = utils.Button(
            button_style="danger", description="Remove simulation"
        )

        self.remove_simulation_button.on_click(self.remove_simulation)

        self.ldos_orbital_maps_image_uploader.observe(
            self.load_ldos_orbital_maps_image, names="_counter"
        )
        ...
        self.children = [
            ...
            self.ldos_orbital_maps_widgets,
    ...
    def load_ldos_orbital_maps_image(self, change):
        ...
        for (
            image_filename,
            image_data,
        ) in self.ldos_orbital_maps_image_uploader.value.items():
            image_binary = image_data["content"]
            image_widget = utils.Image(
                value=image_binary,
                layout=ipw.Layout(
                    border="solid 1px #cccccc", width="250px", height="250px"
                ),
            )

            image_caption = utils.Text(description="Caption: ")

            image_widgets = utils.VBox([image_widget, image_caption])

            image_filepath = f"./data/images/{image_filename}"
    ...
    def remove_simulation(self, b):
        ...
        for idx, simulation_widget in enumerate(simulations_accordion_children):
            if (
                simulation_widget.accordion_parent_item_index
                > self.accordion_parent_item_index
            ):
                simulation_widget.accordion_parent_item_index -= 1
            self.accordion_parent_widget.set_title(
                idx, simulations_accordion_titles[idx]
            )
...
class ExperimentObjectWidget(ipw.VBox):
    def __init__(self, accordion_parent_widget, accordion_parent_item_index):
        ...
        self.doi_label = utils.Label(value="DOI: ")

        self.doi_textbox = utils.Text(layout=ipw.Layout(width="40%"))

        self.doi_widgets = utils.HBox([self.doi_label, self.doi_textbox])

        self.year_label = utils.Label(value="Year: ")

        self.year_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.year_widgets = utils.HBox([self.year_label, self.year_textbox])

        self.comments_label = utils.Label(value="Comments/flags: ")

        self.comments_textareabox = utils.Textarea(layout=ipw.Layout(width="20%"))

        self.comments_widgets = utils.HBox(
            [self.comments_label, self.comments_textareabox]
        )
        ...
        self.band_energies_label = utils.Label(value="Band energies")

        self.bands_energies_items = [
            utils.Label(value="Index"),
            utils.Label(value="Label"),
            utils.Label(value="E - E_F (eV)"),
            utils.Label(value="Error (eV)"),
            utils.Label(value="Effective mass (m_e)"),
            utils.Label(value="Error (m_e)"),
            utils.Text(layout=ipw.Layout(width="80%")),
            utils.Text(layout=ipw.Layout(width="80%")),
            utils.Text(layout=ipw.Layout(width="80%")),
            utils.Text(layout=ipw.Layout(width="80%")),
        ...
        self.relevant_stm_label = utils.Label(value="Relevant STMs:")
        ...
        self.relevant_stm_image_uploader = utils.FileUpload(multiple=True)

        self.relevant_sts_label = utils.Label(value="Relevant STSs:")
        ...
        self.relevant_sts_image_uploader = utils.FileUpload(multiple=True)

        self.precursor_label = utils.Label(value="Precursor")
        ...
        self.precursor_cdxml_uploader = utils.FileUpload(
            accept=".cdxml", multiple=False
        )

        self.precursor_cdxml_uploader_widgets = utils.HBox(
            [self.precursor_cdxml_uploader_label, self.precursor_cdxml_uploader]
        )

        self.precursor_file_hyperlink_label = utils.Label(value="Precursor file:")

        self.precursor_file_hyperlink = utils.HTML(value="")

        self.precursor_file_hyperlink_widgets = utils.HBox(
            [self.precursor_file_hyperlink_label, self.precursor_file_hyperlink]
        )

        self.precursor_cdxml_file = ""
        self.precursor_image_file = ""

        self.precursor_caption_label = utils.Label(value="Precursor caption: ")

        self.precursor_caption_textbox = utils.Text(layout=ipw.Layout(width="20%"))

        self.precursor_caption_widgets = utils.HBox(
            [self.precursor_caption_label, self.precursor_caption_textbox]
        )

        self.remove_experiment_button = utils.Button(
            button_style="danger", description="Remove experiment"
        )

        self.remove_experiment_button.on_click(self.remove_experiment)

        self.relevant_stm_image_uploader.observe(
            self.load_relevant_stm, names="_counter"
        )

        self.relevant_sts_image_uploader.observe(
            self.load_relevant_sts, names="_counter"
        )

        self.precursor_cdxml_uploader.observe(
            self.load_precursor_cdxml, names="_counter"
        )
    ...
    def load_relevant_stm(self, change):
        ...
        for (
            image_filename,
            image_data,
        ) in self.relevant_stm_image_uploader.value.items():
            image_binary = image_data["content"]
            image_widget = utils.Image(
                value=image_binary,
                layout=ipw.Layout(
    ...
    def load_relevant_sts(self, change):
        ...
        for (
            image_filename,
            image_data,
        ) in self.relevant_sts_image_uploader.value.items():
            image_binary = image_data["content"]
            image_widget = utils.Image(
                value=image_binary,
                layout=ipw.Layout(
    ...
    def load_precursor_cdxml(self, change):
        for filename, data in self.precursor_cdxml_uploader.value.items():
            ...
            if generate_molecule_image is False:
                self.precursor_imagebox.value = utils.read_file(
                    "./data/images/white_screen.jpg"
                )
                print("Cannot generate molecule image.")
    ...
    def remove_experiment(self, b):
        ...
        for idx, experiment_widget in enumerate(experiments_accordion_children):
            if (
                experiment_widget.accordion_parent_item_index
                > self.accordion_parent_item_index
            ):
                experiment_widget.accordion_parent_item_index -= 1
            self.accordion_parent_widget.set_title(
                idx, experiments_accordion_titles[idx]
            )
...
class RecordObjectWidget(ipw.VBox):
    def __init__(self):
        # Initialize the parent
        super().__init__()
        ...
        self.add_simulation_button.on_click(self.add_simulation)
    ...
    def get_widgets_data(self):
        ...
        if self.summary_info_widgets.structure_sketch_image_file:
            widgets_data["structure_sketch_file"] = (
                self.summary_info_widgets.structure_sketch_image_file
            )

        if self.summary_info_widgets.structure_cdxml_file:
            widgets_data["structure_file"] = (
                self.summary_info_widgets.structure_cdxml_file
            )

        if self.summary_info_widgets.classification_textbox.value:
            widgets_data["classification"] = (
                self.summary_info_widgets.classification_textbox.value
            )
        ...
        for simulation in simulations_objects:
            ...
            if simulation.uks_multi_textbox.value:
                ...
                if utils.is_integer(uks_multiplicity):
                    ...
                    print("UKS multiplicity is not integer.")
            ...
            if simulation.num_alpha_textbox.value:
                ...
                if utils.is_integer(num_alpha):
                    ...
                    print("Number of alpha is not integer")
            ...
            if simulation.num_beta_textbox.value:
                ...
                if utils.is_integer(num_beta):
                    ...
                    print("Number of beta is not integer")
            ...
            if simulation.energy_au_textbox.value:
                ...
                if utils.is_number(energy_au):
                    ...
                    print("Energy au is not a number")
            ...
            if simulation.energy_ev_textbox.value:
                ...
                if utils.is_number(energy_ev):
                    ...
                    print("Energy eV is not a number")
            ...
            for idx in range(4, len(band_energies_objects), 4):
                ...
                if band_energies_objects[idx].value:
                    ...
                    if utils.is_integer(index):
                        ...
                        print(f"Index of row {idx} is not integer")
                ...
                if band_energies_objects[idx + 2].value:
                    ...
                    if utils.is_number(e_ef_ev):
                        ...
                        print(f"E-E_F (eV) of row {idx} is not a number")
                ...
                if band_energies_objects[idx + 3].value:
                    ...
                    if utils.is_number(effective_mass_m_e):
                        band_energy_dict["effective_mass_m_e"] = float(
                            effective_mass_m_e
                        )
                    else:
                        print(f"Effective mass (m_e) of row {idx} is not a number")
            ...
            for idx, ldos_orbital_map in enumerate(ldos_orbital_maps_objects):
                ...
                if ldos_orbital_map.children[1].value:
                    ldos_orbital_map_dict["caption"] = ldos_orbital_map.children[
                        1
                    ].value
        ...
        for experiment in experiments_objects:
            ...
            if experiment.comments_textareabox.value:
                experiment_dict["comments_flags"] = (
                    experiment.comments_textareabox.value
                )
            ...
            if experiment.spin_excitation_energy_ev_textbox.value:
                spin_excitation_energy_ev = (
                    experiment.spin_excitation_energy_ev_textbox.value
                )
                if utils.is_number(spin_excitation_energy_ev):
                    experiment_dict["spin_excitation_energy_ev"] = float(
                        spin_excitation_energy_ev
                    )
                else:
                    print("Spin excitation energy (eV) is not a number")
            ...
            for idx in range(6, len(band_energies_objects), 6):
                ...
                if band_energies_objects[idx].value:
                    ...
                    if utils.is_integer(index):
                        ...
                        print(f"Index of row {idx} is not a number")
                ...
                if band_energies_objects[idx + 2].value:
                    ...
                    if utils.is_number(e_ef_ev):
                        ...
                        print(f"E-EF (eV) of row {idx} is not a number")
                ...
                if band_energies_objects[idx + 3].value:
                    ...
                    if utils.is_number(error_ev):
                        ...
                        print(f"Error (eV) of row {idx} is not a number")
                ...
                if band_energies_objects[idx + 5].value:
                    ...
                    if utils.is_number(error_m_e):
                        ...
                        print(f"Error (m_e) of row {idx} is not a number")
    ...
    def add_simulation(self, b):
        ...
        simulation_object_widget = SimulationObjectWidget(
            self.simulations_accordion, simulation_index
        )
        simulations_accordion_children.append(simulation_object_widget)
        self.simulations_accordion.set_title(
            num_simulations, f"Simulation {num_simulations + 1}"
        )
    ...
    def add_experiment(self, b):
        ...
        experiment_object_widget = ExperimentObjectWidget(
            self.experiments_accordion, experiment_index
        )
        experiments_accordion_children.append(experiment_object_widget)
        self.experiments_accordion.set_title(
            experiment_index, f"Experiment {num_experiments + 1}"
        )
        self.experiments_accordion.children = experiments_accordion_children

    def load_widgets(self, record_content):
        # Summary of information
        self.summary_info_widgets.names_textbox.value = ";".join(
            map(str, record_content["names"])
        )
        self.summary_info_widgets.structure_sketch_imagebox.value = utils.read_file(
            record_content["structure_sketch_file"]
        )
        structure_file_hyperlink = urllib.parse.quote(record_content["structure_file"])
        self.summary_info_widgets.structure_file_hyperlink.value = f"<a href='{structure_file_hyperlink}' target='_blank'>Download structure cdxml file</a>"
        self.summary_info_widgets.classification_textbox.value = record_content[
            "classification"
        ]

        # Simulations
        simulations_data = record_content.get("simulations", [])
        simulations_objects = []
        for idx, simulation_content in enumerate(simulations_data):
            simulation_object = SimulationObjectWidget(self.simulations_accordion, idx)
            simulation_object.substrate_textbox.value = simulation_content["substrate"]
            simulation_object.uks_multi_textbox.value = str(
                simulation_content.get("uks_multiplicity", "")
            )
            simulation_object.charge_textbox.value = str(
                simulation_content.get("charge", "")
            )
            simulation_object.num_alpha_textbox.value = str(
                simulation_content.get("number_alpha_electrons", "")
            )
            simulation_object.num_beta_textbox.value = str(
                simulation_content.get("number_beta_electrons", "")
            )
            simulation_object.energy_au_textbox.value = str(
                simulation_content.get("energy_au", "")
            )
            simulation_object.energy_ev_textbox.value = str(
                simulation_content.get("energy_ev", "")
            )
            simulation_object.gap_ev_textbox.value = str(
                simulation_content.get("gap_ev", "")
            )
            ...
            for _, ldos_orbital_map in enumerate(ldos_orbital_maps):
                ...
                image_widget = utils.Image(
                    value=utils.read_file(file),
                    layout=ipw.Layout(
                        border="solid 1px #cccccc", width="250px", height="250px"
                    ),
                )

                caption_widget = utils.Text(description="Caption: ", value=caption)

                widgets_vbox = utils.VBox([image_widget, caption_widget])

                ldos_orbital_maps_widgets.append(widgets_vbox)

            simulation_object.ldos_orbital_maps_widgets.children = (
                ldos_orbital_maps_widgets
            )

            # Enter simulation data into simulations accordion
            simulations_objects.append(simulation_object)
            self.simulations_accordion.set_title(idx, f"Simulation {idx+1}")
        ...
        for idx, experiment_content in enumerate(experiments_data):
            ...
            experiment_object.doi_textbox.value = experiment_content["doi"]
            experiment_object.year_textbox.value = str(experiment_content["year"])
            experiment_object.comments_textareabox.value = experiment_content[
                "comments_flags"
            ]
            ...
            for band_idx, band_energy in enumerate(band_energies_data):
                ...
                label = band_energy.get("label", "")
                e_ef_ev = str(band_energy["e_ef_ev"])
                error_ev = str(band_energy.get("error_ev", ""))
                effective_mass_m_e = str(band_energy.get("effective_mass_m_e", ""))
            ...
            for _, relevant_stm_image in enumerate(relevant_stm_images):
                ...
                relevant_stm_image_widgets.append(widgets_vbox)

            experiment_object.relevant_stm_image_widgets.children = (
                relevant_stm_image_widgets
            )
            ...
            for _, relevant_sts_image in enumerate(relevant_sts_images):
                ...
                relevant_sts_image_widgets.append(widgets_vbox)

            experiment_object.relevant_sts_image_widgets.children = (
                relevant_sts_image_widgets
            )

            # Precursor images
            precursor_content = experiment_content.get("precursor", {})
            if precursor_content:
                experiment_object.precursor_imagebox.value = utils.read_file(
                    precursor_content["image_file"]
                )
                precursor_file_hyperlink = precursor_content["structure_file"]
                encoded_path = urllib.parse.quote(precursor_file_hyperlink)
                experiment_object.precursor_file_hyperlink.value = f"<a href='{encoded_path}' target='_blank'>Download precursor cdxml file</a>"
                experiment_object.precursor_caption_textbox.value = precursor_content[
                    "caption"
                ]

            # Enter experiment data into experiments accordion
            experiments_objects.append(experiment_object)
            self.experiments_accordion.set_title(idx, f"Experiment {idx+1}")

        self.experiments_accordion.children = experiments_objects
